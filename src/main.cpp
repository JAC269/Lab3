...
const char *UBIDOTS_TOKEN = "BBFF-j75bg4wGHfhucvuZAmAW1TVhxxNnwY"; // Put here your Ubidots TOKEN
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  val = payload[0];
    ...
    tft.fillCircle(200,100, 10, greenColor); // Dibujar círculo verde
  } else {
    sw1State = false; // Cambiar estado de sw1 a apagado
    tft.fillCircle(200,100, 10, grayColor); // Dibujar círculo gris
  }
    if (boton == '2')
  if ((char)payload[0] == '1') {
    sw2State = true; // Cambiar estado de sw2 a encendido
    tft.fillCircle(220, 120, 10,blueColor); // Dibujar círculo azul
  } else {
    sw2State = false; // Cambiar estado de sw2 a apagado
    tft.fillCircle(220, 120, 10,grayColor); // Dibujar círculo gris
  }
  }
...
void startAP()
{
  ...
  WiFi.softAP("Chaparro_AP", "facil123");
...
void start_STA_client()
{
  ...
  tft.init();
  tft.setRotation(1); // Cambiar la orientación a horizontal
  tft.fillScreen(TFT_BLACK);
  tft.setTextSize(2); // Tamaño del texto
  tft.setTextColor(TFT_WHITE);
  tft.drawString("PRACTICA1", 10, 10); // Agregar título
  tft.setTextSize(1); // Restaurar tamaño del texto
  tft.drawString("Est. Jhon Chaparro", 10, 30); // Agregar nombre
  // Dibujar círculos iniciales en gris
  tft.fillCircle(200,100, 10, grayColor); // Círculo verde
  tft.fillCircle(200, 100, 10,grayColor); // Círculo azul
  // Dibujar círculos iniciales en gris
  tft.fillCircle(220, 120, 10, grayColor);  // Círculo verde
  tft.fillCircle(220, 120, 10, grayColor); // Círculo azul
...
void loop()
{
  if (is_STA_mode()) // Rutina para modo Station (cliente Ubidots)
  {
    ...
    tft.fillRect(10, 60, 120, 40, TFT_BLACK);// Limpiar área de la tabla
    tft.setCursor(10, 60);
    tft.print("Temp: ");
    tft.print(temp);
    tft.print(" C");
    ...
    if (sw1State)
    {
      tft.fillCircle(200, 100, 10, greenColor);
      digitalWrite(LED_PIN1, HIGH); // Encender el LED
    }
    else
    {
      tft.fillCircle(200, 100, 10, grayColor);
      digitalWrite(LED_PIN1, LOW); // Apagar el LED
    }

    if (sw2State)
    {
      tft.fillCircle(220, 120, 10, blueColor);
      digitalWrite(LED_PIN2, HIGH); // Encender el LED
    }
    else
    {
      tft.fillCircle(220, 120, 10, grayColor);
